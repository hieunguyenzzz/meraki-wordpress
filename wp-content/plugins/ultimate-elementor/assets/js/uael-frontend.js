( function( $ ) {
	...
	var WidgetUAELLoginFormHandler = function( $scope, $ ) {
		...
		var redirect_url = form_wrapper.find( '.elementor-form-fields-wrapper' ).data( 'redirect-url' );
				...
				if( ! invalid_field && error_exists.length === 0 ) {
					...
					$.post( ajaxurl, {
						...
						if ( true === response.success ) {
							...
							_printErrorMessages( $scope, password, 'Error: The Password you have entered is incorrect' );
						} else if ( false === response.success && 'Incorrect Username' === response.data ) {
							_printErrorMessages( $scope, username, 'Error: The Username you have entered is incorrect' );
						}
