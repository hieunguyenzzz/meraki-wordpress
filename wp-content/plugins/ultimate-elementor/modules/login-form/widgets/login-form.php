...
class LoginForm extends Common_Widget {
	...
	protected function register_social_controls() {
		...
		if ( ! isset( $integration_options['facebook_app_id'] ) || '' === $integration_options['facebook_app_id'] ) {
			$this->add_control(
				'facebook_app_id_setting',
				array(
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %s admin link */
						'raw'         => sprintf( __( 'Please configure Facebook App ID from <a href="%s" target="_blank" rel="noopener">here</a>.', 'uael' ), $admin_link ),
					'content_classes' => 'elementor-panel-alert elementor-panel-alert-warning',
					'condition'       => array(
						'facebook_login' => 'yes',
					),
		...
		if ( ( isset( $integration_options['google_client_id'] ) && '' !== $integration_options['google_client_id'] ) && ( isset( $integration_options['facebook_app_id'] ) && '' !== $integration_options['facebook_app_id'] ) ) {
			...
			$this->add_control(
				...
				array(
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %s admin link */
					'raw'             => sprintf( __( 'Note: To avoid any issues while logging in with Google or Facebook, make sure you enter the correct Client and APP ID <a href="%s" target="_blank" rel="noopener">here</a>.', 'uael' ), $admin_link ),
					'content_classes' => 'elementor-panel-alert elementor-panel-alert-warning',
					'conditions'      => array(
						'relation' => 'and',
						'terms'    => array(
			...
			$this->start_controls_section(
				...
				$this->add_control(
					...
					array(
						...
						'raw'             => sprintf( __( '%1$s Getting started article » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/new-login-form-widget-for-elementor/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
				...
				$this->add_control(
					...
					array(
						'type'            => Controls_Manager::RAW_HTML,
						/* translators: %1$s doc link */
						'raw'             => sprintf( __( '%1$s How to create a Google Client ID? » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/create-google-client-id-for-login-form-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
						'content_classes' => 'uael-editor-doc',
					)
				);
				...
				$this->add_control(
					...
					array(
						'type'            => Controls_Manager::RAW_HTML,
						/* translators: %1$s doc link */
						'raw'             => sprintf( __( '%1$s How to create a Facebook App ID? » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/create-facebook-app-id-for-login-form-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
						'content_classes' => 'uael-editor-doc',
					)
				);
	 ...
	 */
	public function render_social_login( $position, $is_hidden, $hide_custom_class, $settings, $google_clientid, $facebook_appid ) {

		$is_google_valid = ( 'yes' === $settings['google_login'] && '' !== $google_clientid );
		$is_fb_valid     = ( 'yes' === $settings['facebook_login'] && '' !== $facebook_appid );
		...
		if ( $is_google_valid || $is_fb_valid ) {
			...
			?>
			<div class="uael-login-form-social-wrapper uael-lf-social-<?php echo $position; ?>">
					...
					if ( $is_google_valid ) {
						...
						<div class="elementor-field-group uael-login-form-google">
							<div class="uaelGoogleContentWrapper" id="uael-google-login" data-clientid="<?php echo $google_clientid; ?>">
								...
								<span class="uael-google-text"><?php echo apply_filters( 'uael_login_form_google_button', $google_string ); ?></span>
							</div>
						</div>
					<?php } ?>
					...
					if ( $is_fb_valid ) {
						...
						<div class="elementor-field-group uael-login-form-facebook">
							<div class="uaelFacebookContentWrapper" id="uael-fbLink" data-appid="<?php echo $facebook_appid; ?>">
								...
								<span class="uael-facebook-text"><?php echo apply_filters( 'uael_login_form_facebook_button', $facebook_string ); ?></span>
							</div>
						</div>
					<?php } ?>
	...
	protected function render() {
		...
		if ( is_user_logged_in() && ! $is_editor ) {
			...
			if ( 'yes' === $settings['show_logged_in_message'] ) {
				...
				<?php
				/* translators: %1$s doc link */
				echo sprintf( __( 'You are Logged in as %1$s (<a href="%2$s">Logout</a>)', 'uael' ), $current_user->display_name, wp_logout_url( $logout_redirect ) );
				?>
				</div>
				<?php
			}
		...
		<div class="uael-login-form-wrapper">
			...
			if ( 'top' === $settings['social_position'] ) {
				$this->render_social_login( 'top', $is_hidden, $hide_custom_class, $settings, $google_clientid, $facebook_appid );
			}
			?>
			<?php
			if ( ! $is_hidden ) {
				?>
				<form class="uael-form uael-login-form" method="post" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>">
						...
						<?php if ( 'yes' === $settings['show_remember_me'] ) { ?>
							<div class="elementor-field-type-checkbox elementor-field-group elementor-column elementor-col-100 elementor-remember-me">
								<label for="uael-login-remember-me">
									...
									<span class="uael-login-form-remember"><?php echo __( 'Remember Me', 'uael' ); ?></span>
								</label>
							</div>
						<?php } ?>
			...
			if ( 'bottom' === $settings['social_position'] || $is_hidden ) {
				$this->render_social_login( 'bottom', $is_hidden, $hide_custom_class, $settings, $google_clientid, $facebook_appid );
			}
			?>
		</div>
		<?php
				...
				<div class="uael-login-form uael-form">
					<div class="elementor-form-fields-wrapper">
						...
						<div class="elementor-field-group elementor-column elementor-field-type-submit elementor-col-100">
							<button type="submit" class="uael-login-form-submit elementor-button elementor-size-{{ settings.button_size }}">
